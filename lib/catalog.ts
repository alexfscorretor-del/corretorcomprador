...
export function generateClientCatalog(client: Client, broker: Broker): void {
  ...
  const clientDataStr = btoa(unescape(encodeURIComponent(JSON.stringify({ ...client, properties: sorted }))));
  ...
  // ── Serializa fotos de todos os imóveis num único JSON ──────────────
  const photosMap: Record<string, string[]> = {};
  sorted.forEach(p => { photosMap[p.id] = p.fotos || []; });
  const photosMapB64 = btoa(unescape(encodeURIComponent(JSON.stringify(photosMap))));
  const propertiesB64 = btoa(unescape(encodeURIComponent(JSON.stringify(sorted))));
  ...
  const cards = sorted.map(p => {
    ...
    const stars  = [1,2,3,4,5].map(n =>
      `<span class="star" data-id="${p.id}" data-val="${n}" style="color:${
        (p.rating||0)>=n?'#facc15':'#3f3f46'};font-size:20px;line-height:1;">★</span>`
    ).join('');

    const thumbStrip = nFotos > 1
      ? `<div class="thumb-strip">${(p.fotos||[]).slice(0,6).map((f,i)=>
          `<img src="${f}" alt="" loading="lazy" onclick="LB.open('${p.id}',${i})" title="Ampliar foto ${i+1}">`
        ).join('')}${nFotos>6?`<div class="thumb-more">+${nFotos-6}</div>`:''}</div>`
      : '';

    return `
<div class="card" data-id="${p.id}">
  <div class="card-media" onclick="LB.open('${p.id}',0)" style="cursor:zoom-in" title="Ampliar foto">
    ${imgSrc
      ? `<img src="${imgSrc}" class="card-img" alt="${p.titulo}" loading="lazy">`
      : '<div class="card-img-placeholder"></div>'}
    <span class="compat-badge">${cp}% Compat\u00edvel</span>
    ${nFotos>1?`<span class="fotos-badge">&#128247; ${nFotos} fotos</span>`:''}
    ${imgSrc?'<div class="zoom-hint"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="18" height="18"><circle cx="11" cy="11" r="7"/><path d="m21 21-4.35-4.35M11 8v6M8 11h6"/></svg> Ampliar</div>':''}
  </div>
  <div class="card-body">
    <h3 class="card-title">${p.titulo}</h3>
    <p class="card-price">R$ ${Number(p.preco).toLocaleString('pt-BR',{minimumFractionDigits:2})}</p>
    <p class="card-meta">${p.bairro||'-'} &bull; ${p.tamanho||'?'}m&sup2; &bull; ${p.quartos??0} qtos &bull; ${p.vagas??0} vaga(s)</p>
    <div class="stars-row" data-id="${p.id}">${stars}</div>
    ${thumbStrip}
    <div class="card-actions">
      <button class="btn-detail" onclick="openDetail('${p.id}')">
        <svg class="btn-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M15 3h6v6h-2V6.41l-8.29 8.3-1.42-1.42 8.3-8.29H15V3Z"/><path d="M5 5h7v2H7v10h10v-5h2v7H5V5Z"/></svg>
        Ver detalhes
      </button>
  ...
  // ── Modais de detalhe ────────────────────────────────────────────────
  const detailModals = sorted.map(p => {
    const cp    = calculateCompatibility(client, p);
    const fotos = p.fotos || [];
    const fotosH = fotos.length
      ? `<div class="detail-photos">${fotos.map((f,i)=>
          `<div class="dthumb" onclick="LB.open('${p.id}',${i})" title="Ampliar">
             <img src="${f}" alt="Foto ${i+1}" loading="lazy">
             <div class="dthumb-hint">&#128269;</div>
           </div>`
        ).join('')}</div>`
      : '';
    ...
    const specsH = specs.map(([l,v])=>`<div class="dspec"><strong>${l}</strong><span>${v}</span></div>`).join('');
...
.hero-btn-primary{background:#fff;color:#111}.hero-btn-primary:hover{background:#e4e4e7}
.hero-btn-secondary{background:rgba(255,255,255,.12);border:1px solid rgba(255,255,255,.12);color:#fff}.hero-btn-secondary:hover{background:rgba(255,255,255,.18)}
...
.compat-badge{position:absolute;top:8px;right:8px;background:rgba(229,9,20,.9);color:#fff;border-radius:12px;padding:4px 10px;font-size:12px;font-weight:700;z-index:2}
.fotos-badge{position:absolute;bottom:8px;left:8px;background:rgba(0,0,0,.72);color:#fff;border-radius:8px;padding:3px 9px;font-size:11px;font-weight:600;z-index:2;backdrop-filter:blur(4px)}
.zoom-hint{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;gap:7px;background:rgba(0,0,0,.48);color:#fff;font-size:13px;font-weight:700;opacity:0;transition:opacity .2s;z-index:3;border-radius:14px;pointer-events:none}
...
.card-price{font-size:20px;font-weight:800;color:#ef4444;margin-bottom:6px}
.card-meta{font-size:12px;color:#a1a1aa;margin-bottom:10px;line-height:1.45}
.stars-row{display:flex;align-items:center;gap:3px;margin:8px 0 10px;user-select:none}

/* THUMB STRIP */
.thumb-strip{display:flex;gap:5px;margin-bottom:12px;overflow-x:auto;scrollbar-width:none}
.thumb-strip::-webkit-scrollbar{display:none}
.thumb-strip img{width:54px;height:40px;object-fit:cover;border-radius:7px;flex-shrink:0;cursor:zoom-in;border:2px solid transparent;transition:border-color .15s,transform .15s}
.thumb-strip img:hover{border-color:#e50914;transform:scale(1.1)}
.thumb-more{width:54px;height:40px;border-radius:7px;background:rgba(255,255,255,.1);display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;color:#a1a1aa;flex-shrink:0}
...
.btn-pdf-card{display:flex;align-items:center;justify-content:center;gap:6px;padding:11px 14px;border-radius:14px;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.08);color:#e4e4e7;font-size:13px;font-weight:700;cursor:pointer;transition:.2s;white-space:nowrap}
...
/* MODAL */
.modal-overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,.85);z-index:900;overflow-y:auto;padding:20px}
...
.dthumb{position:relative;border-radius:10px;overflow:hidden;cursor:zoom-in;aspect-ratio:4/3;background:#27272a}
.dthumb img{width:100%;height:100%;object-fit:cover;display:block;transition:transform .25s,filter .25s}
.dthumb:hover img{transform:scale(1.07);filter:brightness(.6)}
.dthumb-hint{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-size:22px;opacity:0;transition:opacity .2s;pointer-events:none}
.dthumb:hover .dthumb-hint{opacity:1}
...
#lb-counter{color:#e4e4e7;font-size:14px;font-weight:600;letter-spacing:.04em}
#lb-close{
  pointer-events:all;
...
#lb-stage{
  ...
  padding:70px 80px 90px;position:relative;
}
#lb-photo{
...
#lb-prev,#lb-next{
  ...
  transition:background .15s,transform .2s;z-index:3;user-select:none;
  line-height:1;
}
#lb-prev{left:16px}#lb-next{right:16px}
#lb-prev:hover,#lb-next:hover{background:rgba(255,255,255,.28);transform:translateY(-50%) scale(1.1)}
#lb-prev:disabled,#lb-next:disabled{opacity:.2;cursor:default}

#lb-bar{
  position:absolute;bottom:0;left:0;right:0;
  display:flex;align-items:center;justify-content:center;
  gap:7px;padding:10px 18px 16px;
  background:linear-gradient(transparent,rgba(0,0,0,.82));
  overflow-x:auto;scrollbar-width:thin;scrollbar-color:rgba(255,255,255,.2) transparent;
}
#lb-bar::-webkit-scrollbar{height:3px}
#lb-bar::-webkit-scrollbar-thumb{background:rgba(255,255,255,.2);border-radius:3px}
.lb-tn{
  flex:0 0 60px;height:44px;border-radius:6px;overflow:hidden;
  cursor:pointer;border:2px solid transparent;
  opacity:.5;transition:opacity .15s,border-color .15s,transform .15s;
}
.lb-tn img{width:100%;height:100%;object-fit:cover;display:block;pointer-events:none}
.lb-tn:hover{opacity:.8}
.lb-tn.lb-cur{border-color:#e50914;opacity:1;transform:scale(1.1)}
...
@media(max-width:700px){
  ...
  #lb-stage{padding:64px 52px 90px}
  ...
  <div class="hero-inner">
    <div class="hero-left">
      ...
      <div class="hero-actions">
        <a href="javascript:void(0)" class="hero-btn hero-btn-primary" onclick="openDetail('${topProp.id}')">&blacktriangleright; Ver Detalhes</a>
...
<!-- LIGHTBOX GLOBAL -->
<div id="lb-overlay" role="dialog" aria-modal="true" aria-label="Visualizador de fotos">
  <div id="lb-top">
  ...
  <div id="lb-stage">
    ...
    <img id="lb-photo" src="" alt="Foto do im\u00f3vel">
...
var ALL_PROPERTIES = JSON.parse(decodeURIComponent(escape(atob('${propertiesB64}'))));
var BROKER_DATA    = JSON.parse(decodeURIComponent(escape(atob('${brokerDataStr}'))));
var PHOTOS_MAP     = JSON.parse(decodeURIComponent(escape(atob('${photosMapB64}'))));
...
var LB = (function(){
  ...
  var photo  = document.getElementById('lb-photo');
  ...
  function show(idx){
    _idx = (idx + _fotos.length) % _fotos.length;
    var total = _fotos.length;
    counter.textContent = (_idx+1)+' / '+total;
    btnPrev.disabled = (_idx === 0);
    btnNext.disabled = (_idx === total-1);

    // Imagem com fade
    photo.classList.add('lb-fade');
    var src = _fotos[_idx];
    setTimeout(function(){
      photo.src = src;
      photo.onload = function(){ photo.classList.remove('lb-fade'); };
      if(photo.complete && photo.naturalWidth) photo.classList.remove('lb-fade');
    }, 100);

    // Miniaturas
    bar.innerHTML = _fotos.map(function(f,i){
      return '<div class="lb-tn'+(i===_idx?' lb-cur':'')
        +'" data-i="'+i+'"><img src="'+f+'" alt=""></div>';
    }).join('');
    bar.querySelectorAll('.lb-tn').forEach(function(t){
      t.addEventListener('click', function(){ show(parseInt(t.dataset.i)); });
    });
    var cur = bar.querySelector('.lb-cur');
    if(cur) cur.scrollIntoView({block:'nearest',inline:'center',behavior:'smooth'});
  }

  function open(propId, startIdx){
    _fotos = PHOTOS_MAP[propId] || [];
    if(!_fotos.length) return;
    // Fecha modais abertos para o lightbox aparecer por cima
  ...
  btnPrev.addEventListener('click', function(){ show(_idx-1); });
  btnNext.addEventListener('click', function(){ show(_idx+1); });

  // Clique no fundo fecha
  overlay.addEventListener('click', function(e){
    if(e.target === overlay || e.target.id === 'lb-stage') close();
  });
  ...
  document.addEventListener('keydown', function(e){
    if(overlay.classList.contains('lb-active')){
      ...
      if(e.key==='ArrowRight' && _idx < _fotos.length-1){ show(_idx+1); return; }
      if(e.key==='ArrowLeft'  && _idx > 0)              { show(_idx-1); return; }
      return;
    }
    if(e.key==='Escape'){
      document.querySelectorAll('.modal-overlay.open').forEach(function(m){
  ...
  overlay.addEventListener('touchend', function(e){
    ...
    if(dx < 0 && _idx < _fotos.length-1) show(_idx+1);
    if(dx > 0 && _idx > 0)              show(_idx-1);
  }, {passive:true});
...
function _applyR(){
  ...
  document.querySelectorAll('.star').forEach(function(el){
    var id=el.dataset.id, v=parseInt(el.dataset.val);
...
function printSingleProperty(id){
  ...
  var sH='<div class="print-specs-grid">'+specs.map(function(s){
  ...
  var bH = '<div class="print-broker-footer"><div class="print-broker-name">'+BROKER_DATA.nome+'</div>'
    +(BROKER_DATA.telefone?'<div>'+BROKER_DATA.telefone+'</div>':'')
    +(BROKER_DATA.email?'<div>'+BROKER_DATA.email+'</div>':'')
    +(BROKER_DATA.empresa?'<div>'+BROKER_DATA.empresa+'</div>':'')+'</div>';
  var ov=document.getElementById('print-single-overlay');
  ov.innerHTML=(compat?'<div class="print-prop-badge">'+compat+'% Compat\u00edvel</div>':'')
