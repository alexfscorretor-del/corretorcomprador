...
export default function PropertyCard({ property, compatibility, onEdit, onDelete, onRating, readonly = false }: Props) {
  ...
  return (
    <>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[110] p-4" onClick={() => setShowDetail(false)}>
          <div className="bg-[#181818] w-full max-w-4xl rounded-3xl max-h-[92vh] overflow-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 md:p-8">
              ...
              {(property.fotos?.length || 0) > 1 && (
                <div className="mb-6">
                  <p className="text-xs text-gray-400 uppercase tracking-wide mb-3">Galeria</p>
                  <div className="grid grid-cols-3 md:grid-cols-4 gap-2">
                    {property.fotos!.map((f, i) => (
                      <div key={i} className="aspect-video rounded-xl overflow-hidden cursor-pointer hover:opacity-80 transition-opacity"
      ...
      {/* ZOOM FOTO */}
      {photoIdx !== null && (
        <div className="fixed inset-0 bg-black/95 flex items-center justify-center z-[200] p-4" onClick={() => setPhotoIdx(null)}>
          <button onClick={() => setPhotoIdx(null)}
            className="absolute top-5 right-5 text-white text-4xl leading-none hover:text-gray-300 bg-transparent border-none cursor-pointer">×</button>
          <img src={property.fotos![photoIdx]} alt="" className="max-w-full max-h-[90vh] rounded-xl object-contain" onClick={e => e.stopPropagation()} />
        </div>
      )}
    </>
  );
