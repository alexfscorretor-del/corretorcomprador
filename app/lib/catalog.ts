...
export function generateClientCatalog(client: Client, broker: Broker): void {
  ...
  const cards = sorted
    ...
    <div class="card" data-id="${p.id}">
      ${
        imgSrc
          ? `<img src="${imgSrc}" class="card-img" alt="${escHtml(p.titulo)}" loading="lazy">`
          : '<div class="card-img-placeholder"></div>'
      }
      <div class="card-body">
        <div class="card-top">
          <span class="compat-badge">${cp}% Compatível</span>
          <span class="price-sm">R$ ${precoFmt}</span>
        </div>
        <h3 class="card-title">${escHtml(p.titulo)}</h3>
        <p class="card-bairro">${escHtml(p.bairro || '')}</p>
        ...
        ${p.descricao ? `<p class="card-desc">${escHtml(p.descricao)}</p>` : ''}
  ...
  const detailModals = sorted
    ...
    <div class="modal-overlay" id="modal-${p.id}" onclick="if(event.target===this)closeDetail('${p.id}')">
      <div class="modal-box">
        ...
        <div class="modal-price">R$ ${precoFmt}</div>
        ${fotosH}
...
<title>Imóveis para ${escHtml(client.nome)}</title>
...
<div class="hero">
  <div class="hero-inner">
    <div class="hero-left">
      ...
      <h1 class="hero-name">${escHtml(topProp.titulo)}</h1>
      <div class="hero-meta">
    ...
    <div class="hero-right">
      ...
      <div class="hero-broker-name">${escHtml(brokerNome)}</div>
      ${brokerEmpresa ? `<div class="hero-broker-sub">${escHtml(brokerEmpresa)}</div>` : ''}
      ${brokerTelefone ? `<div class="hero-broker-phone">${escHtml(brokerTelefone)}</div>` : ''}
      ${brokerEmail ? `<div class="hero-broker-email">${escHtml(brokerEmail)}</div>` : ''}
      <div class="hero-compat">
        ...
        <div class="hero-feat-name">${escHtml(topProp.titulo)}</div>
      </div>
    </div>
  </div>
</div>
